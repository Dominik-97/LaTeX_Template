...
\item nutnost vydefinovat způsob, jakým by na dezinformace měli světové vlády a korporace reagovat.	
\end{enumerate}
...
\hline    
\end{tabular}
  \captionof{table}{Výsledné počty odkazů k vyhledávaní jednotlivých slovních spojení}
  \label{tbl:odkaz}
\end{centering}
\vspace*{5mm}

Například množství odkazů, které vyhledávač od Google zobrazí po zadání hesla \textit{end the lockdown} je více než pět set milionů, je možné důvodně předpokládat, že potenciál výskytu dezinformačního obsahu mezi takovýmto množstvím výsledků bude obrovský., že mezi tímto množstvím obsahu bude 
